...
DROP POLICY IF EXISTS order_financials_select_admin ON secure.order_financials;
CREATE POLICY order_financials_select_admin ON secure.order_financials FOR SELECT USING (public.is_admin());
DROP POLICY IF EXISTS order_financials_insert_rpc ON secure.order_financials;
CREATE POLICY order_financials_insert_rpc ON secure.order_financials FOR INSERT WITH CHECK ((current_setting('app.rpc', true) = 'true') OR public.is_admin());
DROP POLICY IF EXISTS order_financials_update_rpc ON secure.order_financials;
CREATE POLICY order_financials_update_rpc ON secure.order_financials FOR UPDATE USING ((current_setting('app.rpc', true) = 'true') OR public.is_admin());
...
DROP POLICY IF EXISTS webhook_events_select_admin ON secure.webhook_events;
CREATE POLICY webhook_events_select_admin ON secure.webhook_events FOR SELECT USING (public.is_admin());
DROP POLICY IF EXISTS webhook_events_insert_rpc ON secure.webhook_events;
CREATE POLICY webhook_events_insert_rpc ON secure.webhook_events FOR INSERT WITH CHECK ((current_setting('app.rpc', true) = 'true') OR public.is_admin());
DROP POLICY IF EXISTS webhook_events_update_rpc ON secure.webhook_events;
CREATE POLICY webhook_events_update_rpc ON secure.webhook_events FOR UPDATE USING ((current_setting('app.rpc', true) = 'true') OR public.is_admin());

-- 4) Analytics: extend marketplace_daily additively
CREATE SCHEMA IF NOT EXISTS analytics;
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'analytics' AND table_name = 'marketplace_daily') THEN
