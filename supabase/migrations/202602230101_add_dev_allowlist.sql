...
DROP POLICY IF EXISTS dev_allowlist_admin_manage ON public.dev_allowlist;
CREATE POLICY dev_allowlist_admin_manage ON public.dev_allowlist
  FOR ALL USING (public.is_admin()) WITH CHECK (public.is_admin());

-- Allow users to read their own allowlist entry (so Edge functions can verify caller without admin)
DROP POLICY IF EXISTS dev_allowlist_select_owner ON public.dev_allowlist;
CREATE POLICY dev_allowlist_select_owner ON public.dev_allowlist
