  ...
  try {
    ...
    if (profile) {
      if (profile.account_status === 'locked') {
        return makeResponseWithRequestId(JSON.stringify({ success: false, error: { code: "ACCOUNT_LOCKED", message: "Account locked. Contact support." } }), reqId, { status: 403, headers: { ...corsHeaders, "Content-Type": "application/json" } });
      }
      if (profile.account_status === 'restricted') {
        return makeResponseWithRequestId(JSON.stringify({ success: false, error: { code: "PAYMENTS_BLOCKED", message: "Payments temporarily blocked due to suspicious activity. Contact support." } }), reqId, { status: 403, headers: { ...corsHeaders, "Content-Type": "application/json" } });
      }
      if (profile.blocked_until && new Date(profile.blocked_until) > new Date()) {
        return makeResponseWithRequestId(JSON.stringify({ success: false, error: { code: "RATE_LIMITED", message: "Action blocked temporarily" } }), reqId, { status: 429, headers: { ...corsHeaders, "Content-Type": "application/json" } });
      }
    }

    const body = await req.json().catch(() => ({}));
    const { order_id, provider } = body;
    if (!order_id) return new Response(JSON.stringify({ success: false, error: { code: "VALIDATION_ERROR", message: "order_id is required" } }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    ...
    if (rpcRes.error) return makeResponseWithRequestId(JSON.stringify({ success: false, error: { code: "RPC_ERROR", message: rpcRes.error.message } }), reqId, { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    ...
    if (!rzResp.ok) {
      ...
      return makeResponseWithRequestId(JSON.stringify({ success: false, error: { code: "PROVIDER_ERROR", message: "Payment provider error" } }), reqId, { status: 502, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
    ...
    if (attach.error) {
      ...
      return makeResponseWithRequestId(JSON.stringify({ success: false, error: { code: "INTERNAL", message: "Failed to attach provider order" } }), reqId, { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
    ...
    return makeResponseWithRequestId(JSON.stringify({
      success: true,
      data: { key_id: RAZORPAY_KEY_ID, payment_order_id: providerOrderId, amount: amountPaise, currency, order_id }
