...
Deno.serve(async (req: Request) => {
  ...
  try {
    const reqId = getRequestIdFromHeaders(req.headers);
    const authHeader = req.headers.get("Authorization");
    if (!authHeader) return makeResponseWithRequestId(JSON.stringify({ error: "Unauthorized" }), reqId, { status: 401, headers: { ...corsHeaders, "Content-Type": "application/json" } });

    const supabase = createClient(SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY);
    const { data: { user } } = await supabase.auth.getUser(authHeader.replace("Bearer ", ""));
    if (!user) return makeResponseWithRequestId(JSON.stringify({ error: "Unauthorized" }), reqId, { status: 401, headers: { ...corsHeaders, "Content-Type": "application/json" } });

    const body = await req.json().catch(() => ({}));
    const { order_id, provider } = body;
    if (!order_id) return makeResponseWithRequestId(JSON.stringify({ error: "order_id required" }), reqId, { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });
