      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            {t('agent.pendingTransportList.title')}
          </CardTitle>
        </CardHeader>
    ...
    <Card>
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          ...
          <Button variant="ghost" size="sm" onClick={() => navigate(ROUTES.AGENT.TRANSPORT)}>
            {t('agent.pendingTransportList.viewAll')} <ChevronRight className="h-4 w-4 ml-1" />
          </Button>
        </div>
      </CardHeader>
      <CardContent>
        {!pendingRequests || pendingRequests.length === 0 ? (
          <div className="text-center py-8 text-muted-foreground">
            ...
            <p>{t('agent.pendingTransportList.noPending')}</p>
          </div>
        ) : (
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <span className="font-medium">
                    {(request as any).farmer?.full_name || t('agent.pendingTransportList.unknownFarmer')}
                  </span>
