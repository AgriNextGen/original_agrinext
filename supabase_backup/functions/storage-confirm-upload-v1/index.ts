...
Deno.serve(async (req: Request) => {
  ...
  // Optionally check object exists - for now mark ready
  const { data: f } = await admin.from("files").select("id,bucket,object_path,owner_user_id").eq("id", file_id).maybeSingle();
  if (!f) return errorResponse("not_found", "file not found", 404);
  if (f.owner_user_id !== user.id && !(await admin.rpc('is_admin_bool').then(r=>r.data?.[0]))) {
    return errorResponse("forbidden", "not owner", 403);
  }

  await admin.from("files").update({ status: 'ready', updated_at: new Date().toISOString() }).eq("id", file_id);
