...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { trip_id, new_status, proof_file_id, geo_lat, geo_long } = body;
    if (!trip_id || !new_status) return new Response(JSON.stringify({ success:false, error: "trip_id and new_status required" }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });

    const { data, error } = await supabase.rpc("update_trip_status_v1", {
      p_trip_id: trip_id,
      p_new_status: new_status,
      p_proof_file_id: proof_file_id || null,
      p_geo_lat: geo_lat || null,
      p_geo_long: geo_long || null
    });
